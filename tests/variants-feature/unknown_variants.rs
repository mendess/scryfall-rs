...
fn deserialize() {
    ...
    assert_eq!(
        serde_json::from_str::<Layout>(r#""foo""#).unwrap(),
        Layout::Unknown("foo".into())
    );
    assert_eq!(
        serde_json::from_str::<SetType>(r#""foo""#).unwrap(),
        SetType::Unknown("foo".into())
    );
    assert_eq!(
        serde_json::from_str::<PromoType>(r#""foo""#).unwrap(),
        PromoType::Unknown("foo".into())
    );
    assert_eq!(
